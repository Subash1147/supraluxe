    ...
    >
      <Link to={`/product/${product.id}`} className="block">
        ...
        <div className="p-5">
          ...
          <h2 className="mt-3 min-h-[3rem] text-lg font-bold leading-tight text-black">
            {product.title}
          </h2>
          ...
          <p className="mt-4 text-sm text-slate-600">
            <span className="font-semibold text-black">Sizes:</span> {product.sizes?.join(', ')}
          </p>

          <button
            type="button"
            onClick={add}
            className="mt-5 w-full rounded-2xl bg-black px-4 py-3 text-sm font-semibold text-white shadow-[0_2px_8px_rgba(0,0,0,0.12)] transition hover:bg-slate-900 active:scale-95"
